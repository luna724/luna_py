...
def ez_click(x, y, click): # For 1440x900px
...
def ez_clicks(x, y, click): # For 1440x900px
    x = cur.cursor_exchanger(x, 1440, "x")
    y = cur.cursor_exchanger(y, 900, "y")
    pygui.moveTo(x, y)
    if click == True:
...
while end == False: # 終了が渡されるまでループ
    if convert.c_hex("#00CCBB"): # Hex Codeが一致する場合
        ...
        time.sleep(12)
    else: # 不一致の場合 5pixel ずつ下に下がる  # NEXT +125Pixel (Y)
        if donePlayButton == False:
            cur.cursor_
        
        
        while canigonext == False: # 終了が渡されるまで
            ez_click(1020, 260+3, False)
    ...
    #win32api.mouse_event(win32con.MOUSEEVENTF_WHEEL, 0, 0, -50) # ちょっとピッタリ下へ
