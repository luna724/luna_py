...
def webui_tweaks(TYPE):
  if TYPE == "CHARACTER":
    if not len(available_name) < 1:
      return available_name
    else:
      return ["Character LoRA Data Not found.\nPlease Generate from Database Setup"]
...
def def_loraweight_keyword(prompt: str, pattern: str):
  a = 0
...
def preprocessing():
  ...
  def processor(filepath):
    if not os.path.exists(filepath):
      jsoncfg.write(RAW, filepath)
      print(f"jsondata Not Found: {filepath}")
  
  target_filepath =[
    "./database/charactor_lora.json", "./database/charactor_prompt.json",
    "./database/prompt_formatter.json", "./database/sdcp.json",
    "./dataset/prompt_set.json", "./dataset/multi_template.json",
